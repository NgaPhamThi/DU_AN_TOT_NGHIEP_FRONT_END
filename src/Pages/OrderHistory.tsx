            ...
            <div className='flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8'>
              <div className='flex flex-col justify-start items-start dark:bg-gray-800 bg-gray-50 px-4 py-4 md:py-6 md:p-6 xl:p-8 w-full'>
                ...
                {order?.orderDetails?.map((detail) => (
                  <div
                    key={detail.productInfo._id}
                    className='mt-4 md:mt-6 flex flex-col md:flex-row justify-start items-start md:items-center md:space-x-6 xl:space-x-8 w-full'
                  >
                    <div className='pb-4 md:pb-8 w-full md:w-40'>
                      <img className='w-full hidden md:block' src={detail.productInfo.img[0]} alt='dress' />
                      ...
                      <div className='w-full flex flex-col justify-start items-start space-y-8'>
                        ...
                        <div className='flex justify-start items-start flex-col space-y-2'>
                          <p className='text-sm dark:text-white leading-none text-gray-800'>
                            ...
                            {detail.sizeId !== null ? getSizeName(detail.sizeId) : 'N/A'}
                          </p>
                          <p className='text-sm dark:text-white leading-none text-gray-800'>
                            <span className='dark:text-gray-400 text-gray-300'>Color: </span>{' '}
                            {getColorName(detail.colorId)}
                          </p>
                        </div>
                      </div>
                      <div className='flex justify-between space-x-8 items-start w-full'>
                        <p className='text-base dark:text-white xl:text-lg leading-6'>
                          {detail.productInfo.price_sale}{' '}
                          <span className='text-red-300 line-through'> {detail.productInfo.price}Vnđ</span>
                        </p>
                        <p className='text-base dark:text-white xl:text-lg leading-6 text-gray-800'>
                          {detail.quantity}
                        </p>
                        <p className='text-base dark:text-white xl:text-lg font-semibold leading-6 text-gray-800'>
                          {detail.productInfo.price_sale * detail.quantity}
                        </p>
                      </div>
                    </div>
                  </div>
